<div>
    <main class="lg:min-h-screen lg:mt-0 mt-32 flex items-center lg:mb-0 mb-10">
        <div class="container px-8 mx-auto grid lg:grid-cols-2 gap-10 items-center">
            <div class="space-y-4 lg:order-1 order-2">
                <h1 class="sm:text-6xl text-gray-800 text-4xl font-['poppins'] font-black">Where Comfort
                    Meets Adventure</h1>
                <p class="tracking-wide text-gray-600 sm:text-base text-sm">
                    FExperience a world of possibilities at Shalom Lodge. From cozy rooms to versatile event halls,
                    well-equipped gym, lively bar, and delectable dining options – we offer it all and more. Your
                    ultimate retreat awaits.
                </p>
                @auth
                @if (auth()->user()->hasVerifiedEmail())
                @if (!auth()->user()->hasRole('user'))
                <div x-data="{ open: false }"
                    class="flex sm:flex-row flex-col sm:space-x-2 sm:space-y-0 space-y-2 sm:items-end">
                    ...
                    <div x-show="open" style="display: none" x-on:keydown.escape.prevent.stop="open = false"
                        ...
                        <div x-show="open" x-transition.duration.300ms x-on:click="open = false"
                            class="relative min-h-screen flex items-center justify-center p-4">
                            <div x-on:click.stop x-trap.noscroll.inert="open"
                                class="relative max-w-md w-full bg-white rounded-xl p-10 overflow-y-auto space-y-4 text-center">
                                <i class='bx bx-info-circle text-8xl text-blue-600'></i>
                                <h2 class="text-3xl font-bold text-gray-800" :id="$id('modal-title')">You Can't Do It
                                </h2>
                                <p class="tracking-wide text-gray-600 sm:text-base text-sm">
                ...
                <div x-data="{ open: false }"
                    ...
                    <div class="form-control">
                        ...
                        <input class="input" type="date" name="check_in" id="check_in" wire:model='check_in'
                            wire:change='setCheckIn' min="{{ $minCheckIn }}">
                        @error('check_in')
                    ...
                    <div class="form-control">
                        ...
                        <input class="input" type="date" name="check_out" id="check_out" wire:model='check_out'
                            wire:change='setCheckOut' min="{{ $minCheckOut }}">
                        @error('check_out')
                        <span class="invalid">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div x-show="open" style="display: none" x-on:keydown.escape.prevent.stop="open = false"
                        ...
                        <div x-show="open" x-transition.duration.300ms x-on:click="open = false"
                            class="relative min-h-screen flex items-center justify-center p-4">
                            <form method="POST" action="#" wire:submit.prevent='reservation' x-on:click.stop
                                x-trap.noscroll.inert="open"
                                class="relative max-w-xl w-full bg-white rounded-xl p-10 overflow-y-auto space-y-4">
                                <div class="text-center space-y-4">
                                    <i class='bx bx-info-circle text-8xl text-blue-600'></i>
                                    <h2 class="text-3xl font-bold text-gray-800" :id="$id('modal-title')">Choose Your
                                        Favorite Room</h2>
                                    <p class="tracking-wide text-gray-600 sm:text-base text-sm">
                                        If you want to see a list of what rooms we provide, you can see it <a
                                            class="underline" href="{{ route('rooms.index') }}">here</a>.
                                    </p>
                                </div>
                                <div class="grid sm:grid-cols-2 gap-4 items-start">
                                    <div class="form-control">
                                        <label for="check_in_modal" class="label">Check In</label>
                                        <input class="input" type="date" name="check_in" id="check_in_modal"
                                            wire:model='check_in' wire:change='setCheckIn' min="{{ $minCheckIn }}">
                                        @error('check_in')
                                        <span class="invalid">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="form-control">
                                        ...
                                        <input class="input" type="date" name="check_out" id="check_out_modal"
                                            wire:model='check_out' wire:change='setCheckOut' min="{{ $minCheckOut }}">
                                        @error('check_out')
                                        <span class="invalid">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="form-control">
                                        ...
                                        <select class="select" name="selected_room" id="selected_room"
                                            wire:change='setPrice' wire:model='selected_room'>
                                    ...
                                    <div class="form-control">
                                        ...
                                        <input class="input" type="number" name="total_rooms" id="total_rooms"
                                            wire:model='total_rooms' wire:keyup='setPrice'>
                                        @error('total_rooms')
                                        <span class="invalid">{{ $message }}</span>
                                        @enderror
                                    </div>
                                ...
                                <p class="tracking-wide text-gray-600 sm:text-base text-sm">Total price to pay for this
                                    <span class="font-bold">{{ $totalDays }}</span> day stay with <span
                                        class="font-bold">{{ $total_rooms }} {{ $room_name }}</span> rooms is <span
                                        class="font-bold">K{{ $totalPrice }}</span>. <span class="font-bold">Paid
                                        directly at the hotel, not online.</span>
                                </p>
                                @endif
                                <div class="flex space-x-2 justify-center">
                                    <button class="btn">
                                    ...
                                    <button type="button" wire:click="resetAll" x-on:click="open = false"
                ...
                <div x-data="{ open: false }">
                    <div x-show="open" @reservation:created.window="open = true" style="display: none"
                        ...
                        <div x-show="open" x-transition.duration.300ms x-on:click="open = false"
                            ...
                            <div x-on:click.stop x-trap.noscroll.inert="open"
                                ...
                                <div class="text-center space-y-4">
                                    ...
                                    <h2 class="text-3xl font-bold text-gray-800" :id="$id('modal-title')">Reservation
                                        Successfully</h2>
                                    <p class="tracking-wide text-gray-600 sm:text-base text-sm">
                                        You have successfully made a reservation! Wait for confirmation from the
                                        receptionist first. You can view your reservation data on the dashboard.
                                    </p>
                                </div>
                                <div class="flex space-x-2 justify-center">
                    ...
                    <div x-show="open" style="display: none" x-on:keydown.escape.prevent.stop="open = false"
                        ...
                        <div x-show="open" x-transition.duration.300ms x-on:click="open = false"
                            ...
                            <div x-on:click.stop x-trap.noscroll.inert="open"
                                class="relative max-w-md w-full bg-white rounded-xl p-10 overflow-y-auto space-y-4 text-center">
                                <i class='bx bx-info-circle text-8xl text-blue-600'></i>
                                <h2 class="text-3xl font-bold text-gray-800" :id="$id('modal-title')">Email Verification
                                    First</h2>
                                <p class="tracking-wide text-gray-600 sm:text-base text-sm">
                    ...
                    <div x-show="open" style="display: none" x-on:keydown.escape.prevent.stop="open = false"
                        ...
                        <div x-show="open" x-transition.duration.300ms x-on:click="open = false"
                            ...
                            <div x-on:click.stop x-trap.noscroll.inert="open"
                                class="relative max-w-md w-full bg-white rounded-xl p-10 overflow-y-auto space-y-4 text-center">
                                <i class='bx bx-info-circle text-8xl text-blue-600'></i>
                                <h2 class="text-3xl font-bold text-gray-800" :id="$id('modal-title')">Must Login First
                                </h2>
                                <p class="tracking-wide text-gray-600 sm:text-base text-sm">
            ...
            <div class="relative lg:order-2 order-1">
                <span
                    class="absolute inset-0 border-2 z-0 border-gray-800 translate-x-5 -translate-y-5 rounded-tr-2xl rounded-bl-2xl"></span>
                <div class="overflow-hidden rounded-tr-2xl rounded-bl-2xl relative z-10">
                    <img src="{{ asset('img/hero.jpg') }}"
                        class="w-full block hover:scale-110 transition-all duration-300" alt="Hollux Building">
                </div>
            </div>
        </div>
    </main>
        ...
        <div class="container px-8 mx-auto space-y-10">
            <div class="space-y-2">
                <h1
                    ...
                    Our Best Facilities</h1>
                <p class="tracking-wide text-gray-600 sm:text-base text-sm">We offer the best facilities to accompany
                    your rest</p>
            </div>
            <div class="grid lg:grid-cols-3 sm:grid-cols-2 gap-10">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <a href="{{ route('facilities.index', $facility->code) }}"
        ...
        <div class="container px-8 mx-auto lg:space-y-20 space-y-10">
            <div class="space-y-2">
                <h1
                    class="sm:text-5xl text-gray-800 text-3xl font-['poppins'] font-black capitalize after:content-[''] after:block after:w-10 after:h-1 after:bg-gray-800 after:rounded-full">
                    Favorite Room</h1>
                <p class="tracking-wide text-gray-600 sm:text-base text-sm">We provide many types of rooms that you can
                    choose according to your needs. <a href="{{ route('rooms.index') }}" class="underline">Click here to
                        see all options</a></p>.
            </div>
            <div class="grid md:gap-20 gap-10">
                ...
                <div class="flex md:flex-row flex-col lg:gap-20 md:gap-10 gap-6 md:items-center h-full">
                    <span
                        class="lg:text-6xl text-4xl md:block min-w-[1.5rem] hidden text-gray-800 font-['poppins']">{{ $loop->iteration }}</span>
                    <div
                        class="aspect-[8/6] lg:min-w-[24rem] md:min-w-[20rem] lg:max-w-[24rem] md:max-w-[20rem] w-full rounded-tr-2xl rounded-bl-2xl overflow-hidden">
                        <img class="w-full h-full object-cover hover:scale-110 transition-all duration-300"
                    ...
                    <div class="flex flex-col justify-between md:h-full gap-1">
                        ...
                        <span class="flex justify-between items-center">
                            <span
                                class="text-sm text-gray-600">{{ (int) $room->total_rooms -  (int) $room->reservations->count()}}
                                rooms available</span>

                            <a class="group" href="{{ route('rooms.show', $room->code) }}"><i
                                    class='bx bx-right-arrow-alt lg:text-4xl text-3xl text-gray-600 group-hover:translate-x-2 transition-all duration-300'></i></a>
                        </span>
                    </div>
                </div>
                @endforeach
        ...
        <div class="container px-8 mx-auto space-y-6">
            ...
            <div class="md:columns-3 sm:columns-2 gap-4 space-y-4">
                ...
                <div class="overflow-hidden rounded-tr-2xl rounded-bl-2xl">
                    <img class="w-full hover:scale-110 transition-all duration-300"
                        src="{{ asset("storage/$image->image") }}" alt="{{ $image->title }}"
                        title="{{ $image->title }}">
                </div>
                @endforeach
            </div>
    ...
    <section class="lg:my-40 mt-10 mb-20">
        <div class="container px-8 mx-auto space-y-6 text-center">
            <div class="space-y-2">
                <h1
                    class="sm:text-5xl text-gray-800 text-3xl font-['poppins'] font-black capitalize after:content-[''] after:block after:w-10 after:h-1 after:bg-gray-800 after:rounded-full max-w-max mx-auto">
                    Fascinated?</h1>
