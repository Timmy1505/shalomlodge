<section class="mt-28 mb-10">
        ...
        <main class="w-full space-y-6 lg:col-span-8">
            <div class="aspect-[16/9] overflow-hidden rounded-tl-2xl rounded-br-2xl relative z-10">
                <img src="{{ asset('storage/' . $room->image) }}"
                    class="w-full block hover:scale-110 transition-all duration-300" alt="{{ $room->name }}">
            </div>
            <div
                ...
                <div class="flex items-center gap-1 text-gray-800">
                    ...
                    <span class="text-sm capitalize">K{{ $room->price }}/night</span>
                </div>
            </div>
            <div
                class="prose sm:prose-base prose max-w-none prose-img:rounded-tr-xl prose-img:rounded-bl-xl prose-img:w-full">
                <h1>{{ $room->name }}</h1>
                <blockquote>
                    {{ $room->description }}
                </blockquote>
                ...
                <h3 id="{{ $roomFacility->facility->name }}" class="scroll-mt-20">{{ $roomFacility->facility->name }}
                </h3>
                <div class="aspect-[16/9] overflow-hidden rounded-tl-2xl rounded-br-2xl relative z-10 not-prose">
                    <img src="{{ asset('storage/' . $roomFacility->facility->image) }}"
                        class="w-full block hover:scale-110 transition-all duration-300"
                        alt="{{ $roomFacility->facility->name }}">
                </div>
                <blockquote>
        ...
        <aside class="relative lg:col-span-4 space-y-4">
            <div class="space-y-2">
                ...
                <p class="tracking-wide text-gray-600 sm:text-base text-sm">
                    {{ __("Interested in this room? Hurry up and book before it's too late! ") }}<span
                        class="font-bold">{{ $available }}{{ __(' rooms available.') }}</span>
                </p>
            </div>
            <hr>
            @auth
            ...
            <form action="#" method="POST" wire:submit.prevent='reservation' class="grid gap-4">
                <div class="grid lg:grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="form-control">
                        <label for="check_in" class="label">{{ __('Check In') }}</label>
                        <input class="w-full input" type="date" name="check_in" min="{{ $minCheckIn }}"
                            wire:change="setCheckIn" wire:model='check_in' id="check_in" />
                        @error('check_in')
                    ...
                    <div class="form-control">
                        <label for="check_out" class="label">{{ __('Check Out') }}</label>
                        <input class="w-full input" type="date" name="check_out" min="{{ $minCheckOut }}"
                            wire:change="setCheckOut" wire:model='check_out' id="check_out" />
                        @error('check_out')
                        <span class="invalid">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="form-control">
                    <label for="total_rooms" class="label">{{ __('Total Rooms') }}</label>
                    <input class="w-full input" type="number" name="total_rooms" min="1" max="{{ $available }}"
                        wire:keyup='setPrice' wire:model="total_rooms" id="total_rooms" />
                    @error('total_rooms')
                    <span class="invalid">{{ $message }}</span>
                    @enderror
                </div>
                @if ($check_in && $check_out && $total_rooms)
                <p class="tracking-wide text-gray-600 sm:text-base text-sm">Total price to pay for this <span
                        class="font-bold">{{ $totalDays }}</span> day stay with <span
                        class="font-bold">{{ $total_rooms }} {{ $room->name }}</span> rooms is <span
                        class="font-bold">K{{ $totalPrice }}</span>. <span class="font-bold">Paid directly at the hotel,
                        not online.</span></p>
                @endif
            ...
            <div class="space-y-6">
                ...
                <div class="bg-gray-100 p-6 rounded-tr-xl rounded-bl-xl space-y-2">
                    ...
                    <div class="flex items-center gap-2">
                        <img class="w-10 h-10 rounded-tr-xl rounded-bl-xl"
                            src="{{ asset('storage/' . $review->user->avatar) }}" alt="">
    ...
    <div x-data="{ open: false }">
        <div x-show="open" @review:created.window="open = true" @review:edited.window="open = true"
            style="display: none" x-on:keydown.escape.prevent.stop="open = false" role="dialog" aria-modal="true"
            x-id="['modal-title']" :aria-labelledby="$id('modal-title')" class="fixed inset-0 overflow-y-auto z-50">
            <div x-show="open" x-transition.duration.300ms.opacity class="fixed inset-0 bg-black/50"></div>
            <div x-show="open" x-transition.duration.300ms x-on:click="open = false"
                class="relative min-h-screen flex items-center justify-center p-4">
                <div x-on:click.stop x-trap.noscroll.inert="open"
                    ...
                    <div class="text-center space-y-4">
                        ...
                        <p class="tracking-wide text-gray-600 sm:text-base text-sm">Your review has been successfully
                            submitted! Thank you for your review!</p>
                    </div>
                    <div class="flex space-x-2 justify-center">
                        <button type="button" x-on:click="open = false" class="btn">
                            Okay
