<div class="space-y-8">
    ...
    <x-table>
        ...
        <tbody>
            ...
            <tr class="bg-white border-b">
                ...
                <td class="td">
                    <a class="underline"
                        href="{{ route('rooms.show', $reservation->room->code) }}">{{ $reservation->room->name }}</a>
                </td>
                ...
                <td class="td flex space-x-2">
                    <a target="_blank" rel="noopener noreferrer"
                        href="{{ route('dashboard.receptionist.reservations.proof', $reservation->code) }}"
                        class="btn btn-sm">Print</a>
                    @if ($reservation->status === 'waiting')
                    <button wire:click='confirm("{{ $reservation->code }}")'
                        class="btn btn-sm btn-outline">Confirm</button>
                    @endif
                    @if ($reservation->status === 'confirmed')
                    <button wire:click='checkIn("{{ $reservation->code }}")' class="btn btn-sm btn-outline">Check
                        In</button>
                    @endif
                    @if ($reservation->status === 'check in')
                    <button wire:click='checkOut("{{ $reservation->code }}")' class="btn btn-sm btn-outline">Check
                        Out</button>
                    @endif
                </td>
            </tr>
    ...
    <div x-data="{ open: false }">
        <div x-show="open" @status:confirmed.window="open = true" style="display: none"
    ...
    <div x-data="{ open: false }">
        <div x-show="open" @status:checkin.window="open = true" style="display: none"
            x-on:keydown.escape.prevent.stop="open = false" role="dialog" aria-modal="true" x-id="['modal-title']"
            :aria-labelledby="$id('modal-title')" class="fixed inset-0 overflow-y-auto z-50">
            <div x-show="open" x-transition.duration.300ms.opacity class="fixed inset-0 bg-black/50"></div>
            <div x-show="open" x-transition.duration.300ms x-on:click="open = false"
    ...
    <div x-data="{ open: false }">
        <div x-show="open" @status:checkout.window="open = true" style="display: none"
            x-on:keydown.escape.prevent.stop="open = false" role="dialog" aria-modal="true" x-id="['modal-title']"
            :aria-labelledby="$id('modal-title')" class="fixed inset-0 overflow-y-auto z-50">
            <div x-show="open" x-transition.duration.300ms.opacity class="fixed inset-0 bg-black/50"></div>
            <div x-show="open" x-transition.duration.300ms x-on:click="open = false"
                ...
                <div x-on:click.stop x-trap.noscroll.inert="open"
                    ...
                    <div class="text-center space-y-4">
                        <i class='bx bx-check-circle text-8xl text-green-600'></i>
                        <h2 class="text-3xl font-bold text-gray-800" :id="$id('modal-title')">Successfully Check Out
                        </h2>
                    </div>
                    <div class="flex space-x-2 justify-center">
                        <button type="button" x-on:click="open = false" class="btn">
