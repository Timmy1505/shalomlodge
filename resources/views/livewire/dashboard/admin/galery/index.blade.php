<div class="space-y-8">
    ...
    <div x-data='{ open: false }'>
        <div x-show="open" @galery:edited.window="open = false" @galery:edit.window="open = true" style="display: none"
            ...
            <form method="POST" action="#" wire:click='cancel' wire:submit.prevent='update' x-show="open"
                x-transition.duration.300ms x-on:click="open = false"
                ...
                <div x-on:click.stop x-trap.noscroll.inert="open"
                    ...
                    <div class="overflow-hidden rounded-tr-2xl rounded-bl-2xl">
                        <img class="w-full" wire:loading.class="blur-sm" wire:target="new_image"
    ...
    <div x-data='{ open: false }'>
        <div x-show="open" @galery:deleted.window="open = false" @galery:delete.window="open = true"
    ...
    <div class="md:columns-3 sm:columns-2 gap-4 space-y-4">
        ...
        <div class="overflow-hidden rounded-tr-2xl rounded-bl-2xl relative">
            ...
            <img class="w-full hover:scale-110 transition-all duration-300 relative z-0"
                src="{{ asset('storage/' . $galery->image) }}" alt="{{ $galery->title }}" title="{{ $galery->title }}">
        </div>
        @empty
    ...
    <div x-data="{ open: false }">
        <div x-show="open" @galery:created.window="open = true" style="display: none"
            x-on:keydown.escape.prevent.stop="open = false" role="dialog" aria-modal="true" x-id="['modal-title']"
            :aria-labelledby="$id('modal-title')" class="fixed inset-0 overflow-y-auto z-50">
            <div x-show="open" x-transition.duration.300ms.opacity class="fixed inset-0 bg-black/50"></div>
    ...
    <div x-data="{ open: false }">
        <div x-show="open" @galery:edited.window="open = true" style="display: none"
            x-on:keydown.escape.prevent.stop="open = false" role="dialog" aria-modal="true" x-id="['modal-title']"
            :aria-labelledby="$id('modal-title')" class="fixed inset-0 overflow-y-auto z-50">
            <div x-show="open" x-transition.duration.300ms.opacity class="fixed inset-0 bg-black/50"></div>
            <div x-show="open" x-transition.duration.300ms x-on:click="open = false"
    ...
    <div x-data="{ open: false }">
        <div x-show="open" @galery:deleted.window="open = true" style="display: none"
            x-on:keydown.escape.prevent.stop="open = false" role="dialog" aria-modal="true" x-id="['modal-title']"
            :aria-labelledby="$id('modal-title')" class="fixed inset-0 overflow-y-auto z-50">
            <div x-show="open" x-transition.duration.300ms.opacity class="fixed inset-0 bg-black/50"></div>
            <div x-show="open" x-transition.duration.300ms x-on:click="open = false"
