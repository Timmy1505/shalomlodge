<div>
    ...
    <div class="mt-8 grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-4">
        ...
        <div class="py-4 px-6 bg-gray-50 rounded-tr-xl rounded-bl-xl">
            <div class="flex items-center gap-4">
                <div
                    class="h-14 w-14 grid place-items-center rounded-tr-xl rounded-bl-xl bg-gray-800 text-white text-xl">
    ...
    <div class="mt-8 space-y-4">
        <h2 class="text-gray-800 text-xl font-['poppins'] font-bold capitalize">About Shalom Lodge</h2>
        <div class="grid md:grid-cols-2 items-start gap-x-10 gap-y-4">
            <div class="bg-gray-50 p-6 rounded-tr-xl rounded-bl-xl space-y-4">
                <div class="aspect-[16/9] rounded-tr-2xl rounded-bl-2xl overflow-hidden">
                    @if ($image)
                    <img wire:loading.class="blur-sm" wire:target='image'
                        class="w-full h-full object-cover hover:scale-110 transition-all duration-300"
                        src="{{ $image->temporaryUrl() }}" alt="About Hollux">
                    @else
                    <img wire:loading.class="blur-sm" wire:target='image'
                        class="w-full h-full object-cover hover:scale-110 transition-all duration-300"
                        src="{{ asset('storage/' . $about->image) }}" alt="About Hollux">
                    @endif
                </div>
            ...
            <form wire:submit.prevent='update' class="flex flex-col space-y-4" action="#" method="POST">
                <div x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true"
                    ...
                    x-on:livewire-upload-progress="progress = $event.detail.progress" class="grid gap-4">
                ...
                <div class="form-control">
                    ...
                    <input wire:model="title" type="text" value="{{ $about->title }}" name="title" id="title"
                        class="textarea">
                </div>
                <div class="form-control">
                    <label for="text" class="label">Text</label>
                    <textarea wire:model="text" name="text" id="text" rows="6"
                        class="resize-none textarea">{{ $about->text }}</textarea>
                </div>
    ...
    <div x-data="{ open: false }">
        <div x-show="open" @about:updated.window="open = true" style="display: none"
