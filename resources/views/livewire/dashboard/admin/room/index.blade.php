<div class="space-y-8">
    <h1
        class="text-gray-800 text-3xl font-['poppins'] font-black capitalize after:content-[''] after:block after:w-10 after:h-1 after:bg-gray-800 after:rounded-full">
        Room</h1>
        ...
        <div class="space-y-4">
            ...
            <div
                ...
                <div class="flex items-center gap-1 text-gray-800">
                    ...
                    <span class="text-sm capitalize">K{{ $room->price }}</span>
                </div>
            </div>
            <div class="space-y-2">
                <div class="flex justify-between items-center">
                    <h3 class="font-bold font-['poppins'] text-lg text-gray-800">{{ $room->name }}</h3>
                    <a href="{{  url('/dashboard/admin/rooms/' . $room->code) }}"
                        wire:click.prevent='show("{{ $room->code }}")' class="flex items-center gap-1 group">
            ...
            <span
                class="text-sm text-gray-600 bg-gray-200 py-2 text-center rounded-tr-xl rounded-bl-xl block">{{ (int) $room->total_rooms -  (int) $room->reservations->count()}}
    ...
    <div x-data='{ open: false }'>
        <div x-show="open" @room:show.window="open = true" style="display: none"
            x-on:keydown.escape.prevent.stop="open = false" role="dialog" aria-modal="true" x-id="['modal-title']"
            :aria-labelledby="$id('modal-title')" class="fixed inset-0 overflow-y-auto z-50">
            <div x-show="open" x-transition.duration.300ms.opacity class="fixed inset-0 bg-black/50"></div>
            <div wire:click='cancel' x-show="open" x-transition.duration.300ms x-on:click="open = false"
                ...
                <div x-on:click.stop x-trap.noscroll.inert="open"
                    ...
                    <div class="aspect-[16/9] rounded-tl-2xl rounded-br-2xl overflow-hidden w-full">
                        <img class="w-full h-full object-cover hover:scale-110 transition-all duration-300"
                            src="{{ asset('storage/' . $selectedRoom->image) }}" alt="{{ $selectedRoom->name }}">
                    </div>
                    <div
                        class="bg-gray-200 text-sm text-gray-600 flex gap-x-4 gap-y-2 justify-center rounded-tr-xl rounded-bl-xl py-2 px-4">
                        <div class="flex items-center gap-1 text-gray-800">
                            <i class='bx bx-tag-alt'></i>
                    ...
                    <article
                        class="prose sm:prose-base prose-sm max-w-none prose-headings:font-['poppins'] break-words">
                        ...
                        <div class="aspect-[16/9] rounded-tl-2xl rounded-br-2xl overflow-hidden w-full not-prose">
                            <img class="w-full h-full object-cover hover:scale-110 transition-all duration-300"
                                src="{{ asset('storage/' . $facility->facility->image) }}"
                    ...
                    <span
                        class="text-sm text-gray-600 bg-gray-200 py-2 text-center rounded-tr-xl rounded-bl-xl block">{{ (int) $selectedRoom->total_rooms -  (int) $selectedRoom->reservations->count()}}
                        rooms available</span>
    ...
    @endif
    <div x-data='{ open: false }'>
        <div x-show="open" @room:deleted.window="open = false" @room:delete.window="open = true" style="display: none"
            x-on:keydown.escape.prevent.stop="open = false" role="dialog" aria-modal="true" x-id="['modal-title']"
            :aria-labelledby="$id('modal-title')" class="fixed inset-0 overflow-y-auto z-50">
            <div x-show="open" x-transition.duration.300ms.opacity class="fixed inset-0 bg-black/50"></div>
            <div wire:click='cancel' x-show="open" x-transition.duration.300ms x-on:click="open = false"
    ...
    <div x-data="{ open: false }">
        <div x-show="open" @room:deleted.window="open = true" style="display: none"
            x-on:keydown.escape.prevent.stop="open = false" role="dialog" aria-modal="true" x-id="['modal-title']"
            :aria-labelledby="$id('modal-title')" class="fixed inset-0 overflow-y-auto z-50">
            <div x-show="open" x-transition.duration.300ms.opacity class="fixed inset-0 bg-black/50"></div>
